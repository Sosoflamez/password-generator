...
export default function OptionsGrid({ options, optionsRef, onChange }: Props) {
    const opts = options as unknown as Record<string, boolean>;
    ...
    return (
        <div ref={optionsRef} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
            {OPTIONS_CONFIG.map(({ key, label, example }) => (
                <label
                    key={key}
                    style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px", background: opts[key] ? "rgba(0,255,65,0.07)" : "rgba(255,255,255,0.03)", border: `1px solid ${opts[key] ? "rgba(0,255,65,0.35)" : "rgba(255,255,255,0.08)"}`, borderRadius: "10px", cursor: "pointer", transition: "background 0.2s, border-color 0.2s", gridColumn: key === "excludeAmbiguous" ? "span 2" : "auto", opacity: 0 }}
                >
                    <input
                        type="checkbox"
                        name={key}
                    ...
                    />
                    <div>
                        <div style={{ fontSize: "13px", fontWeight: 600, color: opts[key] ? "#00ff41" : "rgba(255,255,255,0.75)" }}>{label}</div>
